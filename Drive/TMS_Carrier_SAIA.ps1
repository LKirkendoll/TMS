...
# Description: Contains functions specific to SAIA operations,
...
# Assumes helper functions like Invoke-SAIAApi, Write-LoadingBar,
# Load-And-Normalize-SAIAData, Get-ReportPath, Select-CsvFile,
# Select-SingleKeyEntry, Get-PermittedKeys are available from TMS_Helpers.ps1 or main script.
# Assumes config variables like $script:saiaApiUri are available from TMS_Config.ps1 (via main script).

function Run-SAIAComparisonReportGUI {
    # GUI VERSION: Generates a report comparing costs between two selected SAIA keys/accounts.
    param(
    ...
    Write-Host "`nRunning SAIA Comparison Report (GUI Mode)..." -ForegroundColor Cyan
    ...
    # --- Data Loading ---
    $shipments = Load-And-Normalize-SAIAData -CsvPath $CsvFilePath # This function returns normalized data including a 'details' array
    if ($shipments -eq $null -or $shipments.Count -eq 0) {
        Write-Warning "No processable SAIA shipment data found in '$CsvFilePath'."
    ...
    $reportFilePath = Get-ReportPath -BaseDir $UserReportsFolder -Username $Username -Carrier 'SAIA' -ReportType 'Comparison' -FilePrefix ($key1NameSafe + "_vs_" + $key2NameSafe)
    if (-not $reportFilePath) { return $null }

    $skippedShipmentCount = 0; $totalDifference = 0.0; $processedShipmentCount = 0
    $reportContent.Add("SAIA Comparison Report"); $reportContent.Add("User: $Username"); $reportContent.Add("Date: $(Get-Date)"); $reportContent.Add("Data File: $CsvFilePath")
    ...
    Write-Host "Processing $totalShipmentsToProcess shipments for SAIA Comparison..."
    $useLoadingBar = Get-Command Write-LoadingBar -ErrorAction SilentlyContinue
    for ($i = 0; $i -lt $totalShipmentsToProcess; $i++) {
        $shipment = $shipments[$i]; $shipmentNumberForDisplay = $i + 1 # $shipment is a PSCustomObject from Load-And-Normalize-SAIAData
        if ($useLoadingBar) { Write-LoadingBar -PercentComplete ([int]($shipmentNumberForDisplay * 100 / $totalShipmentsToProcess)) -Message "Processing Shipment $shipmentNumberForDisplay (SAIA Comp)" }

        $CurrentVerbosePreference = $VerbosePreference; $VerbosePreference = 'SilentlyContinue'
        # Invoke-SAIAApi takes individual parameters extracted from the normalized $shipment object
        $cost1 = Invoke-SAIAApi -OriginZip $shipment.OriginZip -DestinationZip $shipment.DestinationZip `
                                -OriginCity $shipment.OriginCity -OriginState $shipment.OriginState `
                                -DestinationCity $shipment.DestinationCity -DestinationState $shipment.DestinationState `
                                -Weight $shipment.'Total Weight' -Class $shipment.'Freight Class 1' `
                                -Details $shipment.details -KeyData $Key1Data # Pass the details array and KeyData

        $cost2 = Invoke-SAIAApi -OriginZip $shipment.OriginZip -DestinationZip $shipment.DestinationZip `
                                -OriginCity $shipment.OriginCity -OriginState $shipment.OriginState `
                                -DestinationCity $shipment.DestinationCity -DestinationState $shipment.DestinationState `
                                -Weight $shipment.'Total Weight' -Class $shipment.'Freight Class 1' `
                                -Details $shipment.details -KeyData $Key2Data
        $VerbosePreference = $CurrentVerbosePreference

        if ($cost1 -eq $null -or $cost2 -eq $null) {
            $skippedShipmentCount++; Write-Warning "Skipping SAIA shipment $shipmentNumberForDisplay (Origin: $($shipment.OriginZip)) due to API error or no rate."
            continue
        }

        $difference = $cost1 - $cost2; $totalDifference += $difference; $processedShipmentCount++
        $resultsData.Add([PSCustomObject]@{
            OriginZip = $shipment.OriginZip; DestZip = $shipment.DestinationZip; Weight = $shipment.'Total Weight' # Already decimal
            Cost1 = $cost1; Cost2 = $cost2; Difference = $difference
        })
    }
    if ($useLoadingBar) { Write-Progress -Activity "Processing SAIA Comparison Shipments" -Completed }
    ...
    # --- Save Report ---
    try { $reportContent | Out-File -FilePath $reportFilePath -Encoding UTF8 -ErrorAction Stop; Write-Host "`nSAIA Comparison Report saved successfully to: $reportFilePath" -ForegroundColor Green; return $reportFilePath }
    catch { Write-Error "Failed to save SAIA report file '$reportFilePath': $($_.Exception.Message)"; return $null }
}


function Run-SAIAMarginReportGUI {
     # GUI VERSION: Calculates the average margin required for a 'Comparison' account cost to match the average target sell price derived from a 'Base' account cost and its associated margin.
     param(
    ...
    Write-Host "`nRunning SAIA Average Required Margin Report (GUI Mode)..." -ForegroundColor Cyan
    ...
    # --- Data Loading ---
    $shipments = Load-And-Normalize-SAIAData -CsvPath $CsvFilePath
    if ($shipments -eq $null -or $shipments.Count -eq 0) { Write-Warning "No processable SAIA shipment data found in '$CsvFilePath'."; return $null }
    ...
    $reportFilePath = Get-ReportPath -BaseDir $UserReportsFolder -Username $Username -Carrier 'SAIA' -ReportType 'AvgMarginCalc' -FilePrefix ($safeBaseKeyName + "_vs_" + $safeCompKeyName)
    if (-not $reportFilePath) { return $null }

    $skippedShipmentCount = 0; $processedShipmentCount = 0; $totalTargetSellPrice = 0.0; $totalCompCost = 0.0
    $reportContent.Add("SAIA Average Required Margin Calculation Report"); $reportContent.Add("User: $Username"); $reportContent.Add("Date: $(Get-Date)"); $reportContent.Add("Data File: $CsvFilePath")
    ...
    # --- Process Shipments ---
    $totalShipmentsToProcess = $shipments.Count; Write-Host "Processing $totalShipmentsToProcess shipments for SAIA Avg Margin Calc..."
    $useLoadingBar = Get-Command Write-LoadingBar -ErrorAction SilentlyContinue
    for ($i = 0; $i -lt $totalShipmentsToProcess; $i++) {
        $shipment = $shipments[$i]; $shipmentNumberForDisplay = $i + 1
        if ($useLoadingBar) { Write-LoadingBar -PercentComplete ([int]($shipmentNumberForDisplay * 100 / $totalShipmentsToProcess)) -Message "Processing Shipment $shipmentNumberForDisplay (SAIA Avg Margin)" }

        $CurrentVerbosePreference = $VerbosePreference; $VerbosePreference = 'SilentlyContinue'
        $baseCost = Invoke-SAIAApi -OriginZip $shipment.OriginZip -DestinationZip $shipment.DestinationZip `
                                   -OriginCity $shipment.OriginCity -OriginState $shipment.OriginState `
                                   -DestinationCity $shipment.DestinationCity -DestinationState $shipment.DestinationState `
                                   -Weight $shipment.'Total Weight' -Class $shipment.'Freight Class 1' `
                                   -Details $shipment.details -KeyData $BaseKeyData
        $compCost = Invoke-SAIAApi -OriginZip $shipment.OriginZip -DestinationZip $shipment.DestinationZip `
                                   -OriginCity $shipment.OriginCity -OriginState $shipment.OriginState `
                                   -DestinationCity $shipment.DestinationCity -DestinationState $shipment.DestinationState `
                                   -Weight $shipment.'Total Weight' -Class $shipment.'Freight Class 1' `
                                   -Details $shipment.details -KeyData $ComparisonKeyData
        $VerbosePreference = $CurrentVerbosePreference

        if ($baseCost -eq $null -or $compCost -eq $null) { $skippedShipmentCount++; Write-Warning "Skipping SAIA shipment $shipmentNumberForDisplay (API error)."; continue }
        if ($baseCost -le 0) { $skippedShipmentCount++; Write-Warning "Skipping SAIA shipment $shipmentNumberForDisplay (zero/negative Base Cost)."; continue }
        ...
        } catch { Write-Warning "Skipping SAIA shipment $shipmentNumberForDisplay (calculation error: $($_.Exception.Message))"; $skippedShipmentCount++; continue }

        $processedShipmentCount++; $totalTargetSellPrice += $targetSellPrice; $totalCompCost += $compCost
        $resultsData.Add([PSCustomObject]@{
            OriginZip = $shipment.OriginZip; DestZip = $shipment.DestinationZip; Weight = $shipment.'Total Weight'
            BaseCost = $baseCost; CompCost = $compCost; TargetSellPrice = $targetSellPrice; BaseProfit = $baseProfit; CompProfit = $compProfit
        })
    }
     if ($useLoadingBar) { Write-Progress -Activity "Processing SAIA Avg Margin Shipments" -Completed }
    ...
    # --- Save Report ---
    try { $reportContent | Out-File -FilePath $reportFilePath -Encoding UTF8 -ErrorAction Stop; Write-Host "`nSAIA Average Margin Calculation Report saved successfully to: $reportFilePath" -ForegroundColor Green; return $reportFilePath }
    catch { Write-Error "Failed to save SAIA report file '$reportFilePath': $($_.Exception.Message)"; return $null }
}


function Calculate-SAIAMarginForASPReportGUI {
    # GUI VERSION: Calculates the required margin for a specific SAIA account to meet a desired ASP.
    param(
    ...
    Write-Host "`nRunning SAIA Required Margin for Desired ASP Report (GUI Mode)..." -ForegroundColor Cyan
    ...
    $reportFilePath = Get-ReportPath -BaseDir $UserReportsFolder -Username $Username -Carrier 'SAIA' -ReportType 'MarginForASP' -FilePrefix $safeCostKeyName
    if (-not $reportFilePath) { return $null }

    $skippedShipmentCount = 0; $processedShipmentCount = 0; $totalCostValue = 0.0
    $reportContent.Add("SAIA Required Margin for Desired ASP Report"); $reportContent.Add("User: $Username"); $reportContent.Add("Date: $(Get-Date)"); $reportContent.Add("Data File: $CsvFilePath")
    ...
    # --- Process Shipments ---
    $totalShipmentsToProcess = $shipments.Count; Write-Host "Processing $totalShipmentsToProcess shipments for SAIA Margin for ASP..."
    $useLoadingBar = Get-Command Write-LoadingBar -ErrorAction SilentlyContinue
    for ($i = 0; $i -lt $totalShipmentsToProcess; $i++) {
        $shipment = $shipments[$i]; $shipmentNumberForDisplay = $i + 1
        if ($useLoadingBar) { Write-LoadingBar -PercentComplete ([int]($shipmentNumberForDisplay * 100 / $totalShipmentsToProcess)) -Message "Processing Shipment $shipmentNumberForDisplay (SAIA Margin/ASP)" }

        $CurrentVerbosePreference = $VerbosePreference; $VerbosePreference = 'SilentlyContinue'
        $costValue = Invoke-SAIAApi -OriginZip $shipment.OriginZip -DestinationZip $shipment.DestinationZip `
                                    -OriginCity $shipment.OriginCity -OriginState $shipment.OriginState `
                                    -DestinationCity $shipment.DestinationCity -DestinationState $shipment.DestinationState `
                                    -Weight $shipment.'Total Weight' -Class $shipment.'Freight Class 1' `
                                    -Details $shipment.details -KeyData $CostAccountInfo
        $VerbosePreference = $CurrentVerbosePreference

        if ($costValue -eq $null) { $skippedShipmentCount++; Write-Warning "Skipping SAIA shipment $shipmentNumberForDisplay (API error)."; continue }

        $processedShipmentCount++; $totalCostValue += $costValue
        $resultsData.Add([PSCustomObject]@{
            OriginZip = $shipment.OriginZip; DestZip = $shipment.DestinationZip; Weight = $shipment.'Total Weight'; Cost = $costValue
        })
    }
    if ($useLoadingBar) { Write-Progress -Activity "Processing SAIA Margin/ASP Shipments" -Completed }
    ...
    # --- Save Report ---
    try { $reportContent | Out-File -FilePath $reportFilePath -Encoding UTF8 -ErrorAction Stop; Write-Host "`nSAIA Required Margin for ASP Report saved successfully to: $reportFilePath" -ForegroundColor Green; return $reportFilePath }
    catch { Write-Error "Failed to save SAIA report file '$reportFilePath': $($_.Exception.Message)"; return $null }
}


function List-SAIAPermittedKeysGUI {
    # Lists SAIA keys permitted for a given customer profile.
    param(
        [Parameter(Mandatory=$true)][hashtable]$CustomerProfile,
        [Parameter(Mandatory=$true)][hashtable]$AllSAIAKeys
     )
     Write-Host "`n--- SAIA Keys Permitted for Customer: $($CustomerProfile.CustomerName) ---" -ForegroundColor Cyan
     $allowedKeyNames = @()
     if ($CustomerProfile.ContainsKey('AllowedSAIAKeys') -and $CustomerProfile['AllowedSAIAKeys'] -is [array]) {
         $allowedKeyNames = $CustomerProfile['AllowedSAIAKeys']
     }

     if ($allowedKeyNames.Count -eq 0) { Write-Host "No SAIA keys/accounts permitted for this customer."; return }

     $permittedKeys = Get-PermittedKeys -AllKeys $AllSAIAKeys -AllowedKeyNames $allowedKeyNames

     if ($permittedKeys.Count -eq 0) { Write-Warning "Could not retrieve details for any permitted SAIA keys."; return }
     ...
     $permittedKeys.GetEnumerator() | Sort-Object Value.Name | ForEach-Object {
         ...
         if ($keyDetails -is [hashtable]) {
              $keyDetails.GetEnumerator() | Where-Object { $_.Name -ne 'Name' -and $_.Name -ne 'TariffFileName' } | Sort-Object Name | ForEach-Object {
                   $displayValue = if ($_.Name -eq 'Password' -and $_.Value) { '********' } else { $_.Value } # Mask password
...
Write-Verbose "TMS SAIA Carrier Functions loaded."
